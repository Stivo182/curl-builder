...
Процедура СобратьКомандуНаСервере()
	...
	РезультатКоманда = Curl.СобратьКоманду(Объект.URL, ПолучитьОтправляемыеДанные(), МетодЗапросаСборки()); // Строка
...
Процедура ДополнитьСборкуТихимРежимомИОтображениемОшибок(Curl)
	...
	Если Объект.ТихийРежим Тогда
		ДобавитьМетодОбработкиВКод("ИспользоватьТихийРежим");
	КонецЕсли;
	
	Curl.ПоказыватьОшибкиВТихомРежиме(Объект.ПоказыватьОшибкиВТихомРежиме);
	
	Если Объект.ПоказыватьОшибкиВТихомРежиме Тогда
		ДобавитьМетодОбработкиВКод("ПоказыватьОшибкиВТихомРежиме");
	КонецЕсли;
...
Процедура ОбновитьРеквизиты()
	...
	ОбновитьВозможностьРазрешенияНезащищенногоПодключения();
...
&НаКлиенте
Процедура ОбновитьВозможностьРазрешенияНезащищенногоПодключения()

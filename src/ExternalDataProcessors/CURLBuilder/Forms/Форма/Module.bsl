...
&НаКлиенте
Процедура ПроксиВидСертификаУЦПриИзменении(Элемент)
	НастроитьФорму();	
КонецПроцедуры

&НаКлиенте
Процедура ВидСертификаУЦПриИзменении(Элемент)
	НастроитьФорму();	
КонецПроцедуры
...
&НаКлиенте
Процедура ВыполнитьСобркуКоманды(Команда)
	СобратьКомандуНаСервере();
КонецПроцедуры
...
Процедура НастроитьФорму()
	...
	// Главная
	Элементы.МетодЗапроса.Доступность = ЭтоHTTP(СхемаURL);
	...
	// Сохраняемые файлы
	Элементы.ФайлСохраненияЗаголовков.Доступность = ЭтоHTTP(СхемаURL) Или ЭтоFTP(СхемаURL);
	...
	// Прокси
	Элементы.ГруппаПроксиЛогинИПароль.Доступность = Объект.ПроксиМетодАутентификации = МетодАутентификацииBasic()
		Или Объект.ПроксиМетодАутентификации = МетодАутентификацииDigest();
	Элементы.ПроксиСертификатыУдостоверяющихЦентров.Видимость = Объект.ПроксиВидСертификаУЦ = ВидСертификаУЦФайлом();
		
	// Сертификаты
	Элементы.СертификатыУдостоверяющихЦентров.Видимость = Объект.ВидСертификаУЦ = ВидСертификаУЦФайлом();
...
&НаСервере
Процедура УстановитьНадписПодвала()
...
Процедура ОбновитьСвойства()
	УстановитьВозможностьОтправленияДанных();
	УстановитьПризнакОтправкиДанных();
	УстановитьПризнакДоступноMultipart();
КонецПроцедуры

&НаКлиенте
Процедура УстановитьВозможностьОтправленияДанных()
...
&НаКлиенте
Процедура УстановитьПризнакДоступноMultipart()
...
Процедура ДополнитьСборкуАутентификацией(Curl)
	...
	ИначеЕсли Объект.МетодАутентификации = МетодАутентификацииAWS4() Тогда
		...
		ДобавитьМетодОбработкиВКод("АутентификацияAWS4", Объект.КлючДоступаAWS4, Объект.СекретныйКлючAWS4, Объект.РегионAWS4, Объект.СервисAWS4);
	КонецЕсли;
...
// Параметры:
//  Curl - ВнешняяОбработка - Внешняя обработка cURL
//  Настройки - см. СтруктураНастрекДляСборки
//@skip-check dynamic-access-method-not-found
&НаСервере
...
Процедура ДополнитьСборкуПрокси(Curl)
	...
	Если Объект.ПроксиВидСертификаУЦ = ВидСертификаУЦИзОС() Тогда
	...
	ИначеЕсли Объект.ПроксиВидСертификаУЦ = ВидСертификаУЦФайлом() 
...
Процедура ДополнитьСборкуСертификатами(Curl)
	...
	Если Объект.ВидСертификаУЦ = ВидСертификаУЦИзОС() Тогда
	...
	ИначеЕсли Объект.ВидСертификаУЦ = ВидСертификаУЦФайлом() 
...
Процедура ДополнитьСборкуРазрешениемПеренаправлений(Curl)
	...
	Если Объект.РазрешатьПеренаправления Тогда	
		...
		Если Объект.ОтправлятьДанныеАутентификацииНаДругойХост Или ЗначениеЗаполнено(Объект.МаксимальноеКоличествоПеренаправлений) Тогда
...
Процедура ДобавитьМетодОтправитьВКод()
	...
	МетодЗапроса = Объект.МетодЗапроса;
	Если Не ЗначениеЗаполнено(МетодЗапроса) Или МетодЗапроса = "POST" Тогда
		МетодЗапроса = Неопределено;
	КонецЕсли;
	
	ДобавитьМетодОбработкиВКод("Отправить", Объект.URL, ОтправляемыеДанныеТекст, МетодЗапроса);
...
&НаКлиенте
Процедура ЗаполнитьСписокВыбораВидСертификаУЦ()
	
	СписокВыбора = Элементы.ВидСертификаУЦ.СписокВыбора; // СписокЗначений из Строка
	СписокВыбора.Очистить();
	
	СписокВыбора.Добавить("", "Не использовать");
	СписокВыбора.Добавить(ВидСертификаУЦФайлом(), "Указать файл с сертификатами");
	СписокВыбора.Добавить(ВидСертификаУЦИзОС(), "Использовать из ОС");
...
&НаКлиенте
Процедура ЗаполнитьСписокВыбораВидСертификаУЦПрокси()
	
	СписокВыбора = Элементы.ПроксиВидСертификаУЦ.СписокВыбора; // СписокЗначений из Строка
	СписокВыбора.Очистить();
...
&НаКлиентеНаСервереБезКонтекста
Функция ВидСертификаУЦФайлом()
	Возврат "Файлом";	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ВидСертификаУЦИзОС()
	Возврат "ОС";	
КонецФункции
